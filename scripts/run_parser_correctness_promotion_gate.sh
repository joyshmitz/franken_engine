...
replay_command="./scripts/e2e/parser_correctness_promotion_gate_replay.sh ${mode}"
  ...
  case "$mode" in
    check)
      run_step \
        ...
        cargo check -p frankenengine-engine --test parser_correctness_promotion_gate || return 1
      ;;
    test)
      run_step \
        "cargo test -p frankenengine-engine --test parser_correctness_promotion_gate" \
        cargo test -p frankenengine-engine --test parser_correctness_promotion_gate || return 1
      ;;
    clippy)
      run_step \
        "cargo clippy -p frankenengine-engine --test parser_correctness_promotion_gate -- -D warnings" \
        cargo clippy -p frankenengine-engine --test parser_correctness_promotion_gate -- -D warnings || return 1
      ;;
    ci)
      run_step \
        "cargo check -p frankenengine-engine --test parser_correctness_promotion_gate" \
