...
target_dir="${CARGO_TARGET_DIR:-/tmp/rch_target_franken_engine_adversarial_campaign}"
seed="${ADVERSARIAL_CAMPAIGN_SEED:-adversarial-campaign-seed-v1}"
artifact_root="${ADVERSARIAL_CAMPAIGN_ARTIFACT_ROOT:-artifacts/adversarial_campaign}"
...
run_mode() {
  case "$mode" in
    check)
      ...
      ;;
    test)
      run_step "cargo test -p frankenengine-engine --test adversarial_campaign_generator" \
        cargo test -p frankenengine-engine --test adversarial_campaign_generator
      ;;
    clippy)
...
write_manifest() {
  ...
  {
    echo "{"
    echo '  "component": "adversarial_campaign_generator",'
    echo "  \"mode\": \"${mode}\","
    echo "  \"seed\": \"${seed}\","
