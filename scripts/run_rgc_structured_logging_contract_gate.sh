...
policy_id="policy-rgc-structured-logging-contract-v1"
component="rgc_structured_logging_contract"
scenario_id="rgc-054a"
...
write_manifest() {
  ...
  if [[ "$exit_code" -eq 0 ]]; then
    ...
    outcome="fail"
    error_code_json='"FE-RGC-054A-LOG-SCHEMA-0001"'
  fi
  ...
  {
    echo "{\"schema_version\":\"rgc.structured-logging-contract.gate.event.v1\",\"trace_id\":\"${trace_id}\",\"decision_id\":\"${decision_id}\",\"policy_id\":\"${policy_id}\",\"component\":\"${component}\",\"event\":\"gate_completed\",\"scenario_id\":\"${scenario_id}\",\"replay_command\":\"${replay_command}\",\"outcome\":\"${outcome}\",\"error_code\":${error_code_json}}"
  } >"$events_path"

  {
    echo "{"
    echo '  "schema_version": "rgc.structured-logging-contract.gate.run-manifest.v1",'
    echo '  "bead_id": "bd-1lsy.11.16",'
  ...
  echo "rgc structured logging contract manifest: ${manifest_path}"
  echo "rgc structured logging contract events: ${events_path}"
}
