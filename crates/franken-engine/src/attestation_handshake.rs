...
impl CellAuthorization {
    ...
    pub fn canonical_bytes(&self) -> Vec<u8> {
        let mut buf = Vec::new();
        // Length-prefix variable-length fields to prevent collisions
        // in the signed authorization preimage.
        buf.extend_from_slice(self.authorization_id.as_bytes()); // fixed 32 bytes
        buf.extend_from_slice(&(self.cell_id.len() as u64).to_be_bytes());
        buf.extend_from_slice(self.cell_id.as_bytes());
...
mod tests {
    ...
    fn handshake_outcome_serde_all_six_variants() {
        ...
        for v in &variants {
            ...
            let restored: HandshakeOutcome = serde_json::from_str(&json).unwrap();
    ...
    fn reattestation_trigger_serde_all_five_variants() {
        ...
        for v in &variants {
            let json = serde_json::to_string(v).unwrap();
            let restored: ReattestationTrigger = serde_json::from_str(&json).unwrap();
            assert_eq!(*v, restored);
        }
    }
