...
mod tests {
    ...
    fn config_serde_round_trip() {
        ...
        let decoded: GateRunnerConfig = serde_json::from_slice(&json).unwrap();
    ...
    fn gate_evaluation_serde_round_trip() {
        ...
        let decoded: GateEvaluation = serde_json::from_slice(&json).unwrap();
    ...
    fn log_event_serde_round_trip() {
        ...
        let decoded: GateRunnerLogEvent = serde_json::from_slice(&json).unwrap();
    ...
    fn strictness_serde_round_trip() {
        ...
        let decoded: GateStrictness = serde_json::from_slice(&json).unwrap();
        assert_eq!(s, decoded);
    }
    ...
    fn gate_runner_output_serde_round_trip() {
        ...
        let decoded: GateRunnerOutput = serde_json::from_slice(&json).unwrap();
    ...
    fn evidence_artifact_serde_round_trip() {
        ...
        let decoded: EvidenceArtifact = serde_json::from_slice(&json).unwrap();
    ...
    fn equivalence_test_case_serde_roundtrip() {
        ...
        let back: EquivalenceTestCase = serde_json::from_str(&json).unwrap();
        assert_eq!(tc, back);
    ...
    fn performance_measurement_serde_roundtrip() {
        ...
        let back: PerformanceMeasurement = serde_json::from_str(&json).unwrap();
        assert_eq!(pm, back);
    }
    ...
    fn adversarial_test_result_serde_roundtrip() {
        ...
        let back: AdversarialTestResult = serde_json::from_str(&json).unwrap();
        assert_eq!(atr, back);
    }
    ...
    fn gate_runner_input_serde_roundtrip() {
        let input = all_passing_input();
        let json = serde_json::to_string(&input).unwrap();
        let back: GateRunnerInput = serde_json::from_str(&json).unwrap();
    ...
    fn candidate_capability_request_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&req).unwrap();
        let back: CandidateCapabilityRequest = serde_json::from_str(&json).unwrap();
        assert_eq!(req, back);
    }
    ...
    fn gate_runner_output_full_fail_serde_roundtrip() {
        ...
        assert_eq!(output.verdict, GateVerdict::Denied);
        let json = serde_json::to_string(&output).unwrap();
        let back: GateRunnerOutput = serde_json::from_str(&json).unwrap();
