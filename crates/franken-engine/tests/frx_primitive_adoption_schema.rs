...
fn primitive_tier_serde_roundtrip() {
    for tier in [
        ...
        let recovered: PrimitiveTier = serde_json::from_str(&json).unwrap();
...
fn reuse_decision_serde_roundtrip() {
    for decision in [
        ...
        let recovered: ReuseDecision = serde_json::from_str(&json).unwrap();
...
fn primitive_adoption_record_serde_roundtrip() {
    ...
    let recovered: PrimitiveAdoptionRecord = serde_json::from_str(&json).unwrap();
...
fn ev_relevance_risk_score_serde_roundtrip() {
    ...
    let recovered: EvRelevanceRiskScore = serde_json::from_str(&json).unwrap();
...
fn verification_checklist_serde_roundtrip() {
    ...
    let recovered: VerificationChecklist = serde_json::from_str(&json).unwrap();
...
fn fallback_budget_serde_roundtrip() {
    ...
    let recovered: FallbackBudget = serde_json::from_str(&json).unwrap();
...
fn reuse_scan_serde_roundtrip() {
    ...
    let recovered: ReuseScan = serde_json::from_str(&json).unwrap();
