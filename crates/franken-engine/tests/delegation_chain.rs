...
fn delegation_chain_serde_round_trip() {
    ...
    let recovered: DelegationChain = serde_json::from_str(&json).unwrap();
...
fn chain_error_serde_round_trip() {
    ...
    for err in &errors {
        ...
        let recovered: ChainError = serde_json::from_str(&json).unwrap();
...
fn delegation_link_summary_serde_round_trip() {
    ...
    let recovered: DelegationLinkSummary = serde_json::from_str(&json).unwrap();
...
fn enrichment_delegation_verification_context_serde() {
    ...
    let recovered: DelegationVerificationContext = serde_json::from_str(&json).unwrap();
