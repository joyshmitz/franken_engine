...
fn retry_budget_policy_serde_round_trip() {
    ...
    let restored: RetryBudgetPolicy = serde_json::from_str(&json).unwrap();
...
fn retry_budget_policy_serde_from_literal_json() {
    ...
    let policy: RetryBudgetPolicy = serde_json::from_str(json).unwrap();
...
fn fast_path_read_source_serde_fast_path() {
    ...
    let restored: FastPathReadSource = serde_json::from_str(&json).unwrap();
...
fn fallback_reason_serde_retry_budget_exceeded() {
    ...
    let restored: FastPathFallbackReason = serde_json::from_str(&json).unwrap();
...
fn fast_path_read_result_serde_with_fallback_reason() {
    ...
    let restored: FastPathReadResult<u64> = serde_json::from_str(&json).unwrap();
...
fn fast_path_read_result_serde_without_fallback_reason() {
    ...
    let json = serde_json::to_string(&result).unwrap();
    let restored: FastPathReadResult<String> = serde_json::from_str(&json).unwrap();
    assert_eq!(result, restored);
}
...
fn fast_path_telemetry_serde_round_trip() {
    ...
    let restored: FastPathTelemetry = serde_json::from_str(&json).unwrap();
