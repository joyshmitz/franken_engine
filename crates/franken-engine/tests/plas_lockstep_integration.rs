...
fn lockstep_runtime_serde_round_trip_all_variants() {
    for runtime in [
        ...
        let recovered: LockstepRuntime = serde_json::from_str(&json).unwrap();
...
fn lockstep_failure_class_serde_round_trip_all_variants() {
    for failure_class in [
        ...
        let json = serde_json::to_string(&failure_class).unwrap();
        let recovered: LockstepFailureClass = serde_json::from_str(&json).unwrap();
...
fn runtime_observation_serde_round_trip() {
    ...
    let recovered: RuntimeObservation = serde_json::from_str(&json).unwrap();
...
fn runtime_tolerance_default_denies_all_mismatches() {
    ...
    let recovered: RuntimeTolerance = serde_json::from_str(&json).unwrap();
...
fn evaluation_serde_round_trip() {
    ...
    let recovered: PlasLockstepEvaluation = serde_json::from_str(&json).unwrap();
...
fn lockstep_case_serde_round_trip() {
    ...
    let recovered: PlasLockstepCase = serde_json::from_str(&json).unwrap();
...
fn log_event_serde_round_trip() {
    ...
    let recovered: PlasLockstepLogEvent = serde_json::from_str(&json).unwrap();
...
fn runtime_comparison_serde_round_trip() {
    ...
    for comparison in &eval.comparisons {
        ...
        let recovered: RuntimeComparison = serde_json::from_str(&json).unwrap();
...
fn plas_lockstep_error_serde_round_trip() {
    ...
    let recovered: PlasLockstepError = serde_json::from_str(&json).unwrap();
