...
fn matrix_log_event_serde_roundtrip() {
    ...
    let deserialized: MatrixLogEvent = serde_json::from_str(&json).unwrap();
...
fn matrix_log_event_serde_roundtrip_with_all_fields_populated() {
    ...
    let json = serde_json::to_string(&event).unwrap();
    let rt: MatrixLogEvent = serde_json::from_str(&json).unwrap();
    assert_eq!(event, rt);
...
fn matrix_log_event_deserialize_from_known_json() {
    ...
    let event: MatrixLogEvent = serde_json::from_str(json).unwrap();
...
fn matrix_log_event_jsonl_batch_serialization() {
    ...
    for (i, line) in lines.iter().enumerate() {
        let rt: MatrixLogEvent = serde_json::from_str(line).unwrap();
