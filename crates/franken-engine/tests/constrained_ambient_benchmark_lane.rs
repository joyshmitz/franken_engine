...
fn lane_workload_metrics_serde_roundtrip() {
    ...
    let recovered: LaneWorkloadMetrics = serde_json::from_str(&json).unwrap();
...
fn proof_attribution_sample_serde_roundtrip() {
    ...
    let recovered: ProofAttributionSample = serde_json::from_str(&json).unwrap();
...
fn workload_delta_report_serde_roundtrip() {
    ...
    for report in &decision.workload_reports {
        ...
        let recovered: WorkloadDeltaReport = serde_json::from_str(&json).unwrap();
...
fn proof_attribution_report_serde_roundtrip() {
    ...
    for report in &decision.attribution_reports {
        let json = serde_json::to_string(report).unwrap();
        let recovered: ProofAttributionReport = serde_json::from_str(&json).unwrap();
        assert_eq!(*report, recovered);
    }
}
...
fn summary_serde_roundtrip() {
    let decision = run_constrained_ambient_benchmark_lane(&baseline_request());
    let json = serde_json::to_string(&decision.summary).unwrap();
    let recovered: ConstrainedAmbientSummary = serde_json::from_str(&json).unwrap();
...
fn event_serde_roundtrip() {
    ...
    for event in &decision.events {
        ...
        let recovered: ConstrainedAmbientEvent = serde_json::from_str(&json).unwrap();
