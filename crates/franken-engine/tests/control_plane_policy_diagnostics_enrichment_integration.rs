...
fn severity_serde_roundtrip_all() {
    ...
    for s in &severities {
        ...
        let deser: DiagnosticSeverity = serde_json::from_str(&json).unwrap();
...
fn category_serde_roundtrip_all() {
    ...
    for c in &categories {
        ...
        let deser: DiagnosticCategory = serde_json::from_str(&json).unwrap();
...
fn error_code_serde_roundtrip() {
    ...
    let deser: DiagnosticErrorCode = serde_json::from_str(&json).unwrap();
...
fn remediation_guidance_serde_roundtrip() {
    ...
    let deser: RemediationGuidance = serde_json::from_str(&json).unwrap();
...
fn report_serde_roundtrip() {
    ...
    let deser: DiagnosticReport = serde_json::from_str(&json).unwrap();
...
fn control_plane_diagnostic_serde_roundtrip() {
    ...
    let deser: ControlPlaneDiagnostic = serde_json::from_str(&json).unwrap();
...
fn emitter_serde_roundtrip_preserves_state() {
    ...
    let deser: DiagnosticEmitter = serde_json::from_str(&json).unwrap();
