...
fn harness_lane_serde_round_trip_all_variants() {
    for lane in [
        ...
        let recovered: HarnessLane = serde_json::from_str(&json).unwrap();
...
fn baseline_scenario_domain_serde_round_trip_all_variants() {
    for domain in [
        ...
        let recovered: BaselineScenarioDomain = serde_json::from_str(&json).unwrap();
...
fn baseline_scenario_outcome_serde_round_trip() {
    for outcome in [
        ...
        let recovered: BaselineScenarioOutcome = serde_json::from_str(&json).unwrap();
...
fn harness_run_manifest_serde_round_trip() {
    ...
    let recovered: HarnessRunManifest = serde_json::from_str(&json).unwrap();
