...
fn channel_report_serde_roundtrip() {
    ...
    let recovered: ChannelReport = serde_json::from_str(&json).unwrap();
...
fn payload_family_serde_round_trip() {
    for family in PayloadFamily::ALL {
        ...
        let recovered: PayloadFamily = serde_json::from_str(&json).unwrap();
...
fn distortion_metric_serde_round_trip() {
    ...
    for metric in metrics {
        ...
        let recovered: DistortionMetric = serde_json::from_str(&json).unwrap();
...
fn channel_path_serde_round_trip() {
    for path in ChannelPath::ALL {
        ...
        let recovered: ChannelPath = serde_json::from_str(&json).unwrap();
...
fn violation_kind_serde_round_trip() {
    ...
    for kind in kinds {
        ...
        let recovered: ViolationKind = serde_json::from_str(&json).unwrap();
...
fn channel_state_serde_round_trip() {
    ...
    let recovered: ChannelState = serde_json::from_str(&json).unwrap();
...
fn channel_spec_serde_round_trip() {
    ...
    let recovered: ChannelSpec = serde_json::from_str(&json).unwrap();
...
fn policy_violation_serde_round_trip() {
    ...
    let recovered: PolicyViolation = serde_json::from_str(&json).unwrap();
