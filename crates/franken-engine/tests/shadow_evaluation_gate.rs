...
fn shadow_evaluation_candidate_serde_roundtrip() {
    ...
    let recovered: ShadowEvaluationCandidate = serde_json::from_str(&json).unwrap();
...
fn safety_metric_snapshot_serde_roundtrip() {
    ...
    let recovered: SafetyMetricSnapshot = serde_json::from_str(&json).unwrap();
...
fn shadow_extension_class_serde_round_trip() {
    for class in [
        ...
        let recovered: ShadowExtensionClass = serde_json::from_str(&json).unwrap();
...
fn shadow_promotion_verdict_serde_round_trip() {
    for verdict in [
        ...
        let recovered: ShadowPromotionVerdict = serde_json::from_str(&json).unwrap();
...
fn safety_metric_serde_round_trip() {
    for metric in SafetyMetric::ALL {
        ...
        let recovered: SafetyMetric = serde_json::from_str(&json).unwrap();
...
fn shadow_burn_in_threshold_profile_serde_round_trip() {
    ...
    let recovered: ShadowBurnInThresholdProfile = serde_json::from_str(&json).unwrap();
...
fn shadow_evaluation_gate_config_serde_round_trip() {
    ...
    let recovered: ShadowEvaluationGateConfig = serde_json::from_str(&json).unwrap();
...
fn shadow_replay_reference_serde_round_trip() {
    ...
    let recovered: ShadowReplayReference = serde_json::from_str(&json).unwrap();
...
fn human_override_request_serde_round_trip() {
    ...
    let recovered: HumanOverrideRequest = serde_json::from_str(&json).unwrap();
...
fn shadow_evaluation_gate_serde_roundtrip() {
    ...
    let recovered: ShadowEvaluationGate = serde_json::from_str(&json).unwrap();
    assert_eq!(
...
fn feature_field_type_serde_roundtrip() {
    for variant in [
        ...
        let recovered: FeatureFieldType = serde_json::from_str(&json).unwrap();
...
fn clipping_method_serde_roundtrip() {
    for method in [
        ...
        let recovered: ClippingMethod = serde_json::from_str(&json).unwrap();
...
fn composition_method_serde_roundtrip() {
    for method in [
        ...
        let json = serde_json::to_string(&method).unwrap();
        let recovered: CompositionMethod = serde_json::from_str(&json).unwrap();
        assert_eq!(method, recovered);
    }
}
...
fn coordinator_trust_model_serde_roundtrip() {
    for model in [
        ...
        let recovered: CoordinatorTrustModel = serde_json::from_str(&json).unwrap();
...
fn secret_sharing_scheme_serde_roundtrip() {
    for scheme in [SecretSharingScheme::Additive, SecretSharingScheme::Shamir] {
        ...
        let recovered: SecretSharingScheme = serde_json::from_str(&json).unwrap();
...
fn prng_algorithm_serde_roundtrip() {
    ...
    let recovered: PrngAlgorithm = serde_json::from_str(&json).unwrap();
