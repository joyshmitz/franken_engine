...
fn extension_risk_class_serde_round_trip_all_variants() {
    for risk_class in [
        ...
        let json = serde_json::to_string(&risk_class).unwrap();
        let recovered: ExtensionRiskClass = serde_json::from_str(&json).unwrap();
...
fn burn_in_lifecycle_state_serde_round_trip_all_variants() {
    for state in [
        ...
        let recovered: BurnInLifecycleState = serde_json::from_str(&json).unwrap();
...
fn burn_in_failure_code_serde_round_trip_all_variants() {
    for code in [
        ...
        let recovered: BurnInFailureCode = serde_json::from_str(&json).unwrap();
...
fn shadow_observation_serde_round_trip() {
    ...
    let recovered: ShadowObservation = serde_json::from_str(&json).unwrap();
...
fn burn_in_session_config_serde_round_trip() {
    ...
    let recovered: BurnInSessionConfig = serde_json::from_str(&json).unwrap();
...
fn metrics_serde_round_trip() {
    ...
    let recovered: BurnInMetrics = serde_json::from_str(&json).unwrap();
...
fn scorecard_metrics_reflect_session_state() {
    ...
    let recovered: BurnInScorecardMetrics = serde_json::from_str(&json).unwrap();
...
fn decision_artifact_serde_round_trip() {
    ...
    let recovered: BurnInDecisionArtifact = serde_json::from_str(&json).unwrap();
...
fn log_event_serde_round_trip() {
    ...
    let recovered: BurnInLogEvent = serde_json::from_str(&json).unwrap();
...
fn rollback_artifacts_serde_round_trip() {
    ...
    let recovered: RollbackProofArtifacts = serde_json::from_str(&json).unwrap();
...
fn burn_in_thresholds_serde_round_trip() {
    ...
    let recovered: BurnInThresholds = serde_json::from_str(&json).unwrap();
...
fn burn_in_error_serde_round_trip() {
    ...
    for err in &errors {
        ...
        let recovered: BurnInError = serde_json::from_str(&json).unwrap();
