...
fn runtime_pins_serde_round_trip() {
    ...
    let recovered: BenchmarkRuntimePins = serde_json::from_str(&json).unwrap();
...
fn fairness_policy_serde_round_trip() {
    ...
    let recovered: BenchmarkFairnessPolicy = serde_json::from_str(&json).unwrap();
...
fn harness_contract_serde_round_trip() {
    ...
    let recovered: BenchmarkHarnessContract = serde_json::from_str(&json).unwrap();
