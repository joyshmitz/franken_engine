...
fn posterior_uniform_serde_roundtrip() {
    ...
    let json_again = serde_json::to_string(&recovered).unwrap();
...
fn decision_verdict_serde_roundtrip() {
    for verdict in [
        ...
        let recovered: DecisionVerdict = serde_json::from_str(&json).unwrap();
...
fn decision_request_serde_roundtrip() {
    ...
    let recovered: DecisionRequest = serde_json::from_str(&json).unwrap();
...
fn adapter_event_serde_roundtrip() {
    ...
    let recovered: control_plane::AdapterEvent = serde_json::from_str(&json).unwrap();
...
fn fallback_policy_serde_roundtrip() {
    ...
    let recovered: FallbackPolicy = serde_json::from_str(&json).unwrap();
...
fn posterior_uniform_has_equal_weights() {
    ...
    let json_again = serde_json::to_string(&again).unwrap();
    assert_eq!(json, json_again);
}
...
fn multiple_bayesian_updates_produce_different_posteriors() {
    let mut posterior = Posterior::uniform(2);
    let initial = serde_json::to_string(&posterior).unwrap();

    // First update: strongly favor state 0
    posterior.bayesian_update(&[0.9, 0.1]);
    let after_first = serde_json::to_string(&posterior).unwrap();
    ...
    let after_second = serde_json::to_string(&posterior).unwrap();
    assert_ne!(
    ...
    posterior.bayesian_update(&[0.1, 0.9]);
    let after_third = serde_json::to_string(&posterior).unwrap();
    assert_ne!(
