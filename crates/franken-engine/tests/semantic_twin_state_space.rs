...
fn twin_state_snapshot_serde_round_trip() {
    ...
    let recovered: TwinStateSnapshot = serde_json::from_str(&json).unwrap();
...
fn semantic_twin_specification_serde_round_trip() {
    ...
    let recovered: SemanticTwinSpecification = serde_json::from_str(&json).unwrap();
...
fn twin_spec_error_serde_round_trip() {
    ...
    let recovered: TwinSpecError = serde_json::from_str(&json).unwrap();
...
fn twin_state_domain_serde_round_trip() {
    for domain in [
        ...
        let recovered: TwinStateDomain = serde_json::from_str(&json).unwrap();
...
fn twin_signal_source_serde_round_trip_all_variants() {
    for source in [
        ...
        let recovered: TwinSignalSource = serde_json::from_str(&json).unwrap();
...
fn twin_phase_serde_round_trip_all_variants() {
    for phase in [
        ...
        let recovered: TwinPhase = serde_json::from_str(&json).unwrap();
...
fn twin_transition_trigger_serde_round_trip_all_variants() {
    for trigger in [
        ...
        let recovered: TwinTransitionTrigger = serde_json::from_str(&json).unwrap();
...
fn twin_state_variable_spec_serde_round_trip() {
    ...
    let json = serde_json::to_string(&spec).unwrap();
    let recovered: TwinStateVariableSpec = serde_json::from_str(&json).unwrap();
    assert_eq!(spec, recovered);
}
...
fn twin_transition_spec_serde_round_trip() {
    ...
    let json = serde_json::to_string(&spec).unwrap();
    let recovered: TwinTransitionSpec = serde_json::from_str(&json).unwrap();
    assert_eq!(spec, recovered);
}
...
fn twin_measurement_contract_serde_round_trip() {
    ...
    let recovered: TwinMeasurementContract = serde_json::from_str(&json).unwrap();
...
fn twin_assumption_spec_serde_round_trip() {
    ...
    let json = serde_json::to_string(&spec).unwrap();
    let recovered: TwinAssumptionSpec = serde_json::from_str(&json).unwrap();
    assert_eq!(spec, recovered);
}
...
fn twin_falsification_hook_serde_round_trip() {
    ...
    let recovered: TwinFalsificationHook = serde_json::from_str(&json).unwrap();
...
fn twin_state_domain_serializes_to_snake_case() {
    let json = serde_json::to_string(&TwinStateDomain::Workload).unwrap();
    assert_eq!(json, "\"workload\"");
}

#[test]
fn twin_signal_source_serializes_to_snake_case() {
    let json = serde_json::to_string(&TwinSignalSource::RuntimeDecisionCore).unwrap();
    assert_eq!(json, "\"runtime_decision_core\"");
    let json2 = serde_json::to_string(&TwinSignalSource::FrirIr2).unwrap();
    assert_eq!(json2, "\"frir_ir2\"");
}

#[test]
fn twin_phase_serializes_to_snake_case() {
    let json = serde_json::to_string(&TwinPhase::ObserveWorkload).unwrap();
    assert_eq!(json, "\"observe_workload\"");
    let json2 = serde_json::to_string(&TwinPhase::SafeMode).unwrap();
