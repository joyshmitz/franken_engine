...
fn frx_07_2_serde_roundtrip_preserves_strategy() {
    ...
    let deserialized: StrategyContract = serde_json::from_str(&serialized).unwrap();
...
fn frx_07_2_serde_roundtrip_via_pretty_print_preserves_strategy() {
    let strategy = parse_strategy();
    let pretty = serde_json::to_string_pretty(&strategy).unwrap();
    let deserialized: StrategyContract = serde_json::from_str(&pretty).unwrap();
    assert_eq!(strategy, deserialized);
}
