...
fn clipping_strategy_serde_roundtrip() {
    ...
    let json = serde_json::to_string(&clipping).unwrap();
...
fn shadow_gate_config_serde_roundtrip() {
    ...
    let restored: ShadowEvaluationGateConfig = serde_json::from_str(&json).unwrap();
...
fn feature_field_type_serde_roundtrip() {
    for fft in [
        ...
        let json = serde_json::to_string(&fft).unwrap();
