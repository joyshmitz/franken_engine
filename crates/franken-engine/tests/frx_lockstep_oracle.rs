...
fn divergence_class_serde_roundtrip() {
    for class in [
        ...
        let recovered: FrxDivergenceClass = serde_json::from_str(&json).unwrap();
...
fn observable_trace_serde_roundtrip() {
    ...
    let recovered: FrxObservableTrace = serde_json::from_str(&json).unwrap();
...
fn trace_event_serde_roundtrip() {
    ...
    let recovered: FrxTraceEvent = serde_json::from_str(&json).unwrap();
...
fn lockstep_case_result_serde_roundtrip() {
    ...
    let json = serde_json::to_string(&case_result).unwrap();
    let recovered: FrxLockstepCaseResult = serde_json::from_str(&json).unwrap();
...
fn lockstep_report_serde_roundtrip() {
    ...
    let recovered: FrxLockstepReport = serde_json::from_str(&json).unwrap();
