...
fn contract_version_serde_roundtrip() {
    ...
    let restored: ContractVersion = serde_json::from_str(&json).unwrap();
...
fn ev_tier_serde_roundtrip() {
    ...
    for tier in &tiers {
        ...
        let restored: EvTier = serde_json::from_str(&json).unwrap();
...
fn rollout_stage_serde_roundtrip() {
    ...
    for stage in &stages {
        ...
        let restored: RolloutStage = serde_json::from_str(&json).unwrap();
...
fn contract_validation_error_serde_all_variants() {
    ...
    for err in &errors {
        ...
        let restored: ContractValidationError = serde_json::from_str(&json).unwrap();
...
fn evidence_contract_deterministic_serialization() {
    ...
    let json2 = serde_json::to_string(&contract).unwrap();
