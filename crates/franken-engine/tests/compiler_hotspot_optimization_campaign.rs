...
fn hotspot_evidence_deserialization_from_json() {
    ...
    let evidence: HotspotEvidence = serde_json::from_str(json).unwrap();
...
fn metric_vector_deserialization_from_json() {
    ...
    let mv: MetricVector = serde_json::from_str(json).unwrap();
...
fn ev_inputs_deserialization_from_json() {
    ...
    let inputs: EvInputs = serde_json::from_str(json).unwrap();
