...
fn serde_optimization_class_all_variants() {
    ...
    for variant in &variants {
        ...
        let restored: OptimizationClass = serde_json::from_str(&json).unwrap();
...
fn serde_activation_stage_all_variants() {
    ...
    for variant in &variants {
        let json = serde_json::to_string(variant).unwrap();
        let restored: ActivationStage = serde_json::from_str(&json).unwrap();
        assert_eq!(*variant, restored);
    }
}
...
fn serde_signer_role_all_variants() {
    ...
    for variant in &variants {
        let json = serde_json::to_string(variant).unwrap();
        let restored: SignerRole = serde_json::from_str(&json).unwrap();
        assert_eq!(*variant, restored);
    }
}
...
fn serde_receipt_attestation_bindings() {
    ...
    let restored: ReceiptAttestationBindings = serde_json::from_str(&json).unwrap();
...
fn serde_equivalence_verdict_all_variants() {
    ...
    for variant in &variants {
        let json = serde_json::to_string(variant).unwrap();
        let restored: EquivalenceVerdict = serde_json::from_str(&json).unwrap();
        assert_eq!(*variant, restored);
    }
}
...
fn serde_trace_comparison_methodology_all_variants() {
    ...
    for variant in &variants {
        let json = serde_json::to_string(variant).unwrap();
        let restored: TraceComparisonMethodology = serde_json::from_str(&json).unwrap();
        assert_eq!(*variant, restored);
    }
}
