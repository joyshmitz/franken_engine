...
fn two_phase_category_serde_round_trip_all_variants() {
    for category in [
        ...
        let recovered: TwoPhaseCategory = serde_json::from_str(&json).unwrap();
...
fn operation_phase_serde_round_trip_all_variants() {
    for phase in [
        ...
        let recovered: OperationPhase = serde_json::from_str(&json).unwrap();
...
fn leak_policy_serde_round_trip_all_variants() {
    for policy in [LeakPolicy::Lab, LeakPolicy::Production] {
        ...
        let recovered: LeakPolicy = serde_json::from_str(&json).unwrap();
...
fn obligation_error_serde_round_trip_cell_not_running() {
    ...
    let recovered: ObligationIntegrationError = serde_json::from_str(&json).unwrap();
