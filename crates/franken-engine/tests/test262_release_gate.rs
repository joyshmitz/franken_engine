...
fn test262_observed_outcome_serde_round_trip() {
    for outcome in [Test262ObservedOutcome::Pass, Test262ObservedOutcome::Fail] {
        ...
        let recovered: Test262ObservedOutcome = serde_json::from_str(&json).unwrap();
...
fn test262_high_water_mark_serde_round_trip() {
    ...
    let recovered: Test262HighWaterMark = serde_json::from_str(&json).unwrap();
...
fn test262_waiver_reason_serde_round_trip() {
    for reason in [
        ...
        let recovered: Test262WaiverReason = serde_json::from_str(&json).unwrap();
...
fn test262_pin_set_serde_round_trip() {
    ...
    let recovered: Test262PinSet = serde_json::from_str(&json).unwrap();
...
fn test262_profile_serde_round_trip() {
    ...
    let recovered: Test262Profile = serde_json::from_str(&json).unwrap();
...
fn test262_waiver_set_serde_round_trip() {
    let waivers = load_waivers();
    let json = serde_json::to_string(&waivers).unwrap();
    let recovered: Test262WaiverSet = serde_json::from_str(&json).unwrap();
...
fn test262_observed_result_serde_round_trip() {
    ...
    let recovered: Test262ObservedResult = serde_json::from_str(&json).unwrap();
...
fn test262_gate_run_serde_round_trip() {
    ...
    let recovered: Test262GateRun = serde_json::from_str(&json).unwrap();
...
fn test262_runner_config_serde_round_trip() {
    ...
    let recovered: Test262RunnerConfig = serde_json::from_str(&json).unwrap();
...
fn test262_outcome_all_variants_serde_round_trip() {
    for outcome in [
        ...
        let json = serde_json::to_string(&outcome).unwrap();
        let recovered: Test262Outcome = serde_json::from_str(&json).unwrap();
        assert_eq!(outcome, recovered);
    }
}
...
fn test262_outcome_serde_uses_snake_case() {
    let json = serde_json::to_string(&Test262Outcome::Waived).unwrap();
    assert_eq!(json, "\"waived\"");
    let json = serde_json::to_string(&Test262Outcome::Timeout).unwrap();
    assert_eq!(json, "\"timeout\"");
    let json = serde_json::to_string(&Test262Outcome::Crash).unwrap();
    assert_eq!(json, "\"crash\"");
}

#[test]
fn test262_observed_outcome_serde_uses_snake_case() {
    let json = serde_json::to_string(&Test262ObservedOutcome::Pass).unwrap();
    assert_eq!(json, "\"pass\"");
    let json = serde_json::to_string(&Test262ObservedOutcome::Fail).unwrap();
    assert_eq!(json, "\"fail\"");
    let json = serde_json::to_string(&Test262ObservedOutcome::Timeout).unwrap();
    assert_eq!(json, "\"timeout\"");
    let json = serde_json::to_string(&Test262ObservedOutcome::Crash).unwrap();
    assert_eq!(json, "\"crash\"");
}
...
fn test262_waiver_serde_round_trip() {
    ...
    let recovered: Test262Waiver = serde_json::from_str(&json).unwrap();
...
fn test262_log_event_serde_round_trip() {
    ...
    let recovered: Test262LogEvent = serde_json::from_str(&json).unwrap();
...
fn test262_run_summary_serde_round_trip() {
    ...
    let recovered: Test262RunSummary = serde_json::from_str(&json).unwrap();
...
fn test262_pass_regression_warning_serde_round_trip() {
    ...
    let recovered: Test262PassRegressionWarning = serde_json::from_str(&json).unwrap();
...
fn test262_collected_artifacts_serde_round_trip() {
    ...
    let recovered: Test262CollectedArtifacts = serde_json::from_str(&json).unwrap();
...
fn deterministic_worker_assignment_serde_round_trip() {
    ...
    let recovered: DeterministicWorkerAssignment = serde_json::from_str(&json).unwrap();
...
fn test262_profile_include_serde_round_trip() {
    ...
    let recovered: Test262ProfileInclude = serde_json::from_str(&json).unwrap();
...
fn test262_profile_exclude_serde_round_trip() {
    ...
    let recovered: Test262ProfileExclude = serde_json::from_str(&json).unwrap();
...
fn waiver_reason_serde_snake_case_values() {
    assert_eq!(
        serde_json::to_string(&Test262WaiverReason::HarnessGap).unwrap(),
        "\"harness_gap\""
    );
    assert_eq!(
        serde_json::to_string(&Test262WaiverReason::HostHookMissing).unwrap(),
        "\"host_hook_missing\""
    );
    assert_eq!(
        serde_json::to_string(&Test262WaiverReason::IntentionalDivergence).unwrap(),
        "\"intentional_divergence\""
    );
    assert_eq!(
        serde_json::to_string(&Test262WaiverReason::NotYetImplemented).unwrap(),
        "\"not_yet_implemented\""
    );
}
