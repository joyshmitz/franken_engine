...
fn gc_phase_serde_round_trip_all_variants() {
    for phase in [GcPhase::Mark, GcPhase::Sweep, GcPhase::Complete] {
        ...
        let decoded: GcPhase = serde_json::from_str(&json).unwrap();
...
fn gc_event_serde_round_trip() {
    ...
    let decoded: GcEvent = serde_json::from_str(&json).unwrap();
...
fn gc_config_serde_round_trip() {
    ...
    let decoded: GcConfig = serde_json::from_str(&json).unwrap();
...
fn gc_error_serde_round_trip_all_variants() {
    ...
    for err in &errors {
        ...
        let decoded: GcError = serde_json::from_str(&json).unwrap();
...
fn extension_heap_serde_round_trip() {
    ...
    let decoded: ExtensionHeap = serde_json::from_str(&json).unwrap();
...
fn gc_collector_serde_round_trip() {
    ...
    let restored: GcCollector = serde_json::from_str(&json).unwrap();
...
fn serialize_and_continue_lifecycle() {
    ...
    // Serialize
    let json = serde_json::to_string(&gc).unwrap();
    let mut restored: GcCollector = serde_json::from_str(&json).unwrap();
