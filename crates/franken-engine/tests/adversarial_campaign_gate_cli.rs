...
fn suppression_gate_cli_passes_with_fixture_input_and_writes_report() {
    ...
    assert_eq!(stdout_json["result"]["passed"], serde_json::Value::Bool(true));
    assert_eq!(
    ...
    assert_eq!(report_json["result"]["passed"], serde_json::Value::Bool(true));
...
fn suppression_gate_cli_returns_exit_two_when_gate_fails() {
    let mut input_json: serde_json::Value = serde_json::from_slice(
    ...
    let stdout_json: serde_json::Value =
        serde_json::from_slice(&output.stdout).expect("stdout should be valid json report");
    assert_eq!(stdout_json["result"]["passed"], serde_json::Value::Bool(false));
    assert!(
