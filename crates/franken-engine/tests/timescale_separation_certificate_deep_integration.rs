...
fn serde_snapshot_with_extreme_values() {
    ...
    let back: PairTelemetrySnapshot = serde_json::from_str(&json).unwrap();
    assert_eq!(s, back);
}
...
fn serde_signal_with_extreme_trigger_values() {
    ...
    let back: BifurcationSignal = serde_json::from_str(&json).unwrap();
...
fn serde_config_with_custom_values() {
    ...
    let back: BifurcationDetectorConfig = serde_json::from_str(&json).unwrap();
