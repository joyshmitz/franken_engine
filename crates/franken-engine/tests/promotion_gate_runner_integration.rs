...
fn gate_kind_serde_round_trip_all_variants() {
    for gate in GateKind::all() {
        ...
        let decoded: GateKind = serde_json::from_str(&json).unwrap();
...
fn gate_strictness_serde_round_trip_all_gates() {
    for gate in GateKind::all() {
        ...
        let decoded: GateStrictness = serde_json::from_str(&json).unwrap();
        assert_eq!(s, decoded);
    }
}
...
fn equivalence_test_case_serde_round_trip() {
    ...
    let decoded: EquivalenceTestCase = serde_json::from_str(&json).unwrap();
...
fn capability_request_serde_round_trip() {
    ...
    let decoded: CandidateCapabilityRequest = serde_json::from_str(&json).unwrap();
...
fn performance_measurement_serde_round_trip() {
    ...
    let decoded: PerformanceMeasurement = serde_json::from_str(&json).unwrap();
    assert_eq!(m, decoded);
}
...
fn adversarial_test_result_serde_round_trip() {
    ...
    let decoded: AdversarialTestResult = serde_json::from_str(&json).unwrap();
    assert_eq!(r, decoded);
}
...
fn config_serde_round_trip() {
    ...
    let decoded: GateRunnerConfig = serde_json::from_str(&json).unwrap();
...
fn gate_runner_input_serde_round_trip() {
    ...
    let decoded: GateRunnerInput = serde_json::from_str(&json).unwrap();
...
fn evidence_artifact_serde_round_trip() {
    ...
    let decoded: EvidenceArtifact = serde_json::from_str(&json).unwrap();
...
fn gate_runner_output_serde_round_trip() {
    ...
    let decoded: GateRunnerOutput = serde_json::from_str(&json).unwrap();
...
fn log_event_serde_round_trip() {
    ...
    let decoded: GateRunnerLogEvent = serde_json::from_str(&json).unwrap();
...
fn serde_round_trip_full_pipeline_output() {
    ...
    let config2: GateRunnerConfig = serde_json::from_str(&config_json).unwrap();
    let input2: GateRunnerInput = serde_json::from_str(&input_json).unwrap();
    let output2: GateRunnerOutput = serde_json::from_str(&output_json).unwrap();
