...
fn execution_wave_all_const_covers_four_waves() {
    ...
    for wave in ExecutionWave::ALL {
        ...
        let recovered: ExecutionWave = serde_json::from_str(&json).unwrap();
...
fn anti_stall_action_serde_round_trip_all_variants() {
    for action in [
        ...
        let recovered: AntiStallAction = serde_json::from_str(&json).unwrap();
...
fn wave_handoff_package_serde_round_trip() {
    ...
    let recovered: WaveHandoffPackage = serde_json::from_str(&json).unwrap();
...
fn protocol_serde_round_trip() {
    let protocol = default_rgc_execution_wave_protocol();
    let json = serde_json::to_string(&protocol).unwrap();
    let recovered: ExecutionWaveProtocol = serde_json::from_str(&json).unwrap();
...
fn wave_plan_entry_serde_round_trip() {
    ...
    let recovered: WavePlanEntry = serde_json::from_str(&json).unwrap();
...
fn file_reservation_protocol_serde_round_trip() {
    let protocol = default_rgc_execution_wave_protocol();
    let json = serde_json::to_string(&protocol.file_reservation).unwrap();
    let recovered: FileReservationProtocol = serde_json::from_str(&json).unwrap();
...
fn agent_mail_protocol_serde_round_trip() {
    let protocol = default_rgc_execution_wave_protocol();
    let json = serde_json::to_string(&protocol.agent_mail).unwrap();
    let recovered: AgentMailProtocol = serde_json::from_str(&json).unwrap();
...
fn anti_stall_thresholds_serde_round_trip() {
    let protocol = default_rgc_execution_wave_protocol();
    let json = serde_json::to_string(&protocol.anti_stall).unwrap();
    let recovered: AntiStallThresholds = serde_json::from_str(&json).unwrap();
...
fn coordination_event_serde_round_trip() {
    ...
    for event in &report.events {
        ...
        let recovered: CoordinationEvent = serde_json::from_str(&json).unwrap();
...
fn dry_run_report_serde_round_trip() {
    ...
    let recovered: CoordinationDryRunReport = serde_json::from_str(&json).unwrap();
