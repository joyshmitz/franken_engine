...
fn scorecard_outcome_serde_round_trip() {
    for outcome in [
        ...
        let recovered: GovernanceScorecardOutcome = serde_json::from_str(&json).unwrap();
...
fn attested_receipt_observation_serde_round_trip() {
    ...
    let recovered: AttestedReceiptObservation = serde_json::from_str(&json).unwrap();
...
fn trend_point_serde_round_trip() {
    ...
    let recovered: GovernanceScorecardTrendPoint = serde_json::from_str(&json).unwrap();
...
fn privacy_budget_health_input_serde_round_trip() {
    ...
    let recovered: PrivacyBudgetHealthInput = serde_json::from_str(&json).unwrap();
...
fn moonshot_governor_health_input_serde_roundtrip() {
    let request = baseline_request();
    let json = serde_json::to_string(&request.moonshot_governor).unwrap();
    let recovered: MoonshotGovernorHealthInput = serde_json::from_str(&json).unwrap();
    assert_eq!(
...
fn cross_repo_conformance_input_serde_roundtrip() {
    let request = baseline_request();
    let json = serde_json::to_string(&request.conformance).unwrap();
    let recovered: CrossRepoConformanceInput = serde_json::from_str(&json).unwrap();
