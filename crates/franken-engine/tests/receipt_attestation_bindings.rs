...
fn attested_receipt_round_trip_through_transparency_log_and_verifier() {
    ...
    let transparency_log = [serde_json::to_vec(&signed).unwrap()];

    let restored: OptReceipt =
        serde_json::from_slice(transparency_log.last().expect("entry")).unwrap();
...
fn validity_window_serde_roundtrip() {
    ...
    let recovered: AttestationValidityWindow = serde_json::from_str(&json).unwrap();
...
fn attestation_bindings_serde_roundtrip() {
    ...
    let recovered: ReceiptAttestationBindings = serde_json::from_str(&json).unwrap();
...
fn opt_receipt_full_serde_roundtrip() {
    ...
    let recovered: OptReceipt = serde_json::from_str(&json).unwrap();
...
fn optimization_class_serde_roundtrip() {
    for class in [
        ...
        let recovered: OptimizationClass = serde_json::from_str(&json).unwrap();
