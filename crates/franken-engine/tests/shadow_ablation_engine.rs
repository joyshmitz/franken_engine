...
fn observation_serde_roundtrip() {
    ...
    let restored: ShadowAblationObservation = serde_json::from_str(&json).unwrap();
...
fn log_event_serde_roundtrip() {
    ...
    let restored: ShadowAblationLogEvent = serde_json::from_str(&json).unwrap();
...
fn evaluation_record_serde_roundtrip() {
    ...
    let restored: ShadowAblationEvaluationRecord = serde_json::from_str(&json).unwrap();
...
fn candidate_request_serde_roundtrip() {
    ...
    let restored: ShadowAblationCandidateRequest = serde_json::from_str(&json).unwrap();
...
fn signed_transcript_serde_roundtrip() {
    ...
    let json = serde_json::to_string(&result.transcript).unwrap();
