...
fn sequence_policy_serde_round_trip() {
    for policy in [SequencePolicy::Strict, SequencePolicy::Monotonic] {
        ...
        let rt: SequencePolicy = serde_json::from_str(&json).unwrap();
        assert_eq!(policy, rt);
    }
}
...
fn channel_payload_serde_round_trip_all_variants() {
    ...
    for payload in payloads {
        ...
        let rt: ChannelPayload = serde_json::from_str(&json).unwrap();
...
fn aead_algorithm_serde_round_trip() {
    for algo in [
        ...
        let rt: AeadAlgorithm = serde_json::from_str(&json).unwrap();
        assert_eq!(algo, rt);
    }
}
...
fn session_config_serde_round_trip() {
    ...
    let rt: SessionConfig = serde_json::from_str(&json).unwrap();
    assert_eq!(config, rt);
}
