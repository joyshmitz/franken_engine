...
fn conformance_asset_record_serde_roundtrip_with_ifc_fields() {
    ...
    let back: conformance_harness::ConformanceAssetRecord = serde_json::from_str(&json).unwrap();
...
fn conformance_waiver_set_serde_roundtrip() {
    ...
    let back: conformance_harness::ConformanceWaiverSet = serde_json::from_str(&json).unwrap();
...
fn waiver_reason_code_serde_roundtrip_all_variants() {
    ...
    for variant in &variants {
        ...
        let back: conformance_harness::WaiverReasonCode = serde_json::from_str(&json).unwrap();
...
fn conformance_runner_config_default_serde_roundtrip() {
    ...
    let back: conformance_harness::ConformanceRunnerConfig = serde_json::from_str(&json).unwrap();
...
fn conformance_repro_metadata_default_serde_roundtrip() {
    ...
    let back: conformance_harness::ConformanceReproMetadata = serde_json::from_str(&json).unwrap();
    assert_eq!(meta, back);
...
fn conformance_delta_kind_serde_roundtrip_all_variants() {
    ...
    for variant in &variants {
        let json = serde_json::to_string(variant).unwrap();
        let back: conformance_harness::ConformanceDeltaKind = serde_json::from_str(&json).unwrap();
        assert_eq!(*variant, back);
    }
}
...
fn conformance_log_event_serde_roundtrip() {
    ...
    let back: conformance_harness::ConformanceLogEvent = serde_json::from_str(&json).unwrap();
...
fn conformance_run_summary_serde_roundtrip() {
    ...
    let back: conformance_harness::ConformanceRunSummary = serde_json::from_str(&json).unwrap();
