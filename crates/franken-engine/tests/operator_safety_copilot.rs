...
fn time_sensitivity_serde_round_trip() {
    for ts in [
        ...
        let recovered: TimeSensitivity = serde_json::from_str(&json).unwrap();
...
fn operator_role_serde_round_trip() {
    for role in [
        ...
        let recovered: OperatorRole = serde_json::from_str(&json).unwrap();
...
fn boundary_trigger_direction_serde_round_trip() {
    for dir in [
        ...
        let recovered: BoundaryTriggerDirection = serde_json::from_str(&json).unwrap();
...
fn evidence_strength_serde_round_trip() {
    ...
    let recovered: EvidenceStrength = serde_json::from_str(&json).unwrap();
...
fn confidence_band_serde_round_trip() {
    ...
    let recovered: ConfidenceBand = serde_json::from_str(&json).unwrap();
...
fn copilot_error_serde_round_trip_all_variants() {
    ...
    for err in &errors {
        ...
        let recovered: CopilotError = serde_json::from_str(&json).unwrap();
...
fn incident_severity_serde_round_trip() {
    for sev in [
        ...
        let recovered: IncidentSeverity = serde_json::from_str(&json).unwrap();
...
fn extension_trust_level_serde_round_trip() {
    for level in [
        ...
        let recovered: ExtensionTrustLevel = serde_json::from_str(&json).unwrap();
...
fn enrichment_rollback_receipt_input_serde_round_trip() {
    ...
    let recovered: RollbackReceiptInput = serde_json::from_str(&json).unwrap();
...
fn enrichment_operator_identity_clone_and_debug_and_serde() {
    ...
    let recovered: OperatorIdentity = serde_json::from_str(&json).unwrap();
...
fn enrichment_calibration_point_serde_round_trip_and_debug() {
    ...
    let recovered: CalibrationPoint = serde_json::from_str(&json).unwrap();
...
fn enrichment_copilot_input_serde_round_trip() {
    ...
    let recovered: OperatorSafetyCopilotInput = serde_json::from_str(&json).unwrap();
...
fn enrichment_decision_boundary_hint_serde_round_trip() {
    ...
    let recovered: DecisionBoundaryHint = serde_json::from_str(&json).unwrap();
