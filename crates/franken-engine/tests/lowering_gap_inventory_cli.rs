...
fn lowering_gap_inventory_serde_roundtrip() {
    ...
    let recovered: LoweringGapInventory = serde_json::from_str(&json).unwrap();
...
fn lowering_gap_descriptor_serde_roundtrip() {
    for site in LoweringGapSiteId::ALL {
        ...
        let recovered: LoweringGapSiteDescriptor = serde_json::from_str(&json).unwrap();
...
fn lowering_gap_inventory_json_pretty_roundtrip() {
    ...
    let recovered: LoweringGapInventory = serde_json::from_str(&pretty).unwrap();
    assert_eq!(inventory.sites.len(), recovered.sites.len());
...
fn lowering_gap_stage_serde_roundtrip() {
    ...
    for stage in stages {
        ...
        let recovered: LoweringGapStage = serde_json::from_str(&json).unwrap();
...
fn lowering_gap_status_serde_roundtrip() {
    ...
    for status in statuses {
        ...
        let recovered: LoweringGapStatus = serde_json::from_str(&json).unwrap();
