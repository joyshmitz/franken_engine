...
fn inventory_serde_round_trip() {
    ...
    let restored: ZeroPlaceholderInventory = serde_json::from_str(&json).unwrap();
...
fn finding_serde_round_trip() {
    ...
    for finding in &inventory.findings {
        ...
        let restored: ZeroPlaceholderFinding = serde_json::from_str(&json).unwrap();
...
fn severity_serde_roundtrip() {
    for sev in [
        ...
        let recovered: ZeroPlaceholderSeverity = serde_json::from_str(&json).unwrap();
...
fn status_serde_roundtrip() {
    for status in [
        ...
        let recovered: ZeroPlaceholderStatus = serde_json::from_str(&json).unwrap();
...
fn subsystem_serde_roundtrip() {
    for sub in [
        ...
        let recovered: ZeroPlaceholderSubsystem = serde_json::from_str(&json).unwrap();
