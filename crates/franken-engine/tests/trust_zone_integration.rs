...
fn zone_create_request_serde_round_trip() {
    ...
    let restored: ZoneCreateRequest = serde_json::from_str(&json).unwrap();
...
fn zone_transition_request_serde_round_trip() {
    ...
    let json = serde_json::to_string(&req).unwrap();
    let restored: ZoneTransitionRequest = serde_json::from_str(&json).unwrap();
    assert_eq!(req, restored);
}
...
fn zone_event_serde_round_trip() {
    ...
    let restored: ZoneEvent = serde_json::from_str(&json).unwrap();
...
fn zone_event_types_serde_round_trip() {
    ...
    for t in &types {
        ...
        let restored: ZoneEventType = serde_json::from_str(&json).unwrap();
...
fn zone_event_outcomes_serde_round_trip() {
    ...
    for o in &outcomes {
        ...
        let restored: ZoneEventOutcome = serde_json::from_str(&json).unwrap();
...
fn trust_zone_error_serde_round_trip_all_variants() {
    ...
    for err in &errors {
        ...
        let restored: TrustZoneError = serde_json::from_str(&json).unwrap();
...
fn trust_zone_serde_round_trip() {
    ...
    let restored: TrustZone = serde_json::from_str(&json).unwrap();
...
fn zone_hierarchy_serde_round_trip() {
    ...
    let restored: ZoneHierarchy = serde_json::from_str(&json).unwrap();
