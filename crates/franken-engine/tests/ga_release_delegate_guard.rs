...
fn slot_kind_serde_round_trip_all_variants() {
    for kind in [
        ...
        let recovered: SlotKind = serde_json::from_str(&json).unwrap();
...
fn slot_capability_serde_round_trip_all_variants() {
    for cap in [
        ...
        let recovered: SlotCapability = serde_json::from_str(&json).unwrap();
...
fn authority_envelope_serde_round_trip() {
    ...
    let recovered: AuthorityEnvelope = serde_json::from_str(&json).unwrap();
...
fn promotion_status_serde_round_trip_all_variants() {
    ...
    for status in variants {
        ...
        let recovered: PromotionStatus = serde_json::from_str(&json).unwrap();
...
fn ga_release_guard_verdict_serde_round_trip() {
    for verdict in [GaReleaseGuardVerdict::Pass, GaReleaseGuardVerdict::Blocked] {
        ...
        let recovered: GaReleaseGuardVerdict = serde_json::from_str(&json).unwrap();
...
fn ga_signed_lineage_artifact_serde_round_trip() {
    ...
    let recovered: GaSignedLineageArtifact = serde_json::from_str(&json).unwrap();
...
fn ga_release_guard_config_serde_round_trip() {
    ...
    let recovered: GaReleaseGuardConfig = serde_json::from_str(&json).unwrap();
...
fn slot_registry_new_is_empty() {
    ...
    let recovered: SlotRegistry = serde_json::from_str(&json).unwrap();
...
fn slot_id_serde_round_trip() {
    ...
    let recovered: SlotId = serde_json::from_str(&json).unwrap();
...
fn ga_release_guard_input_serde_round_trip() {
    ...
    let recovered: GaReleaseGuardInput = serde_json::from_str(&json).unwrap();
...
fn promotion_transition_serde_roundtrip_all_variants() {
    for transition in [
        ...
        let recovered: PromotionTransition = serde_json::from_str(&json).unwrap();
...
fn lineage_event_serde_roundtrip() {
    ...
    let recovered: LineageEvent = serde_json::from_str(&json).unwrap();
    ...
    let json2 = serde_json::to_string(&event_no_receipt).unwrap();
    let recovered2: LineageEvent = serde_json::from_str(&json2).unwrap();
...
fn slot_entry_serde_roundtrip() {
    ...
    let recovered: SlotEntry = serde_json::from_str(&json).unwrap();
...
fn release_slot_class_display_and_serde() {
    ...
    for class in [ReleaseSlotClass::Core, ReleaseSlotClass::NonCore] {
        ...
        let recovered: ReleaseSlotClass = serde_json::from_str(&json).unwrap();
...
fn slot_registry_error_serde_roundtrip() {
    ...
    let recovered: SlotRegistryError = serde_json::from_str(&json).unwrap();
...
fn ga_release_guard_error_serde_roundtrip() {
    ...
    let json = serde_json::to_string(&err).unwrap();
    let recovered: GaReleaseGuardError = serde_json::from_str(&json).unwrap();
    assert_eq!(err, recovered);
}
...
fn slot_replacement_signal_serde_roundtrip() {
    ...
    let recovered: SlotReplacementSignal = serde_json::from_str(&json).unwrap();
...
fn replacement_progress_error_serde_roundtrip() {
    ...
    let json = serde_json::to_string(&err).unwrap();
    let recovered: ReplacementProgressError = serde_json::from_str(&json).unwrap();
    assert_eq!(err, recovered);
}
...
fn core_slot_exemption_serde_roundtrip() {
    ...
    let recovered: CoreSlotExemption = serde_json::from_str(&json).unwrap();
