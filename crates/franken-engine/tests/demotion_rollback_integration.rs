...
fn demotion_reason_serde_round_trip_all_variants() {
    ...
    for r in &reasons {
        ...
        let rt: DemotionReason = serde_json::from_str(&json).unwrap();
        assert_eq!(*r, rt);
    }
}
...
fn demotion_severity_serde_round_trip() {
    for s in [
        ...
        let rt: DemotionSeverity = serde_json::from_str(&json).unwrap();
        assert_eq!(s, rt);
    }
}
...
fn demotion_receipt_serde_round_trip() {
    ...
    let rt: DemotionReceipt = serde_json::from_str(&json).unwrap();
...
fn demotion_policy_serde_round_trip() {
    ...
    let rt: DemotionPolicy = serde_json::from_str(&json).unwrap();
    assert_eq!(p, rt);
}
...
fn auto_demotion_monitor_serde_round_trip() {
    ...
    let rt: AutoDemotionMonitor = serde_json::from_str(&json).unwrap();
    assert_eq!(m, rt);
}
...
fn monitoring_observation_serde_round_trip_all_variants() {
    ...
    for obs in &observations {
        ...
        let rt: MonitoringObservation = serde_json::from_str(&json).unwrap();
        assert_eq!(*obs, rt);
    }
}
...
fn trigger_evaluation_serde_round_trip() {
    ...
    let rt: TriggerEvaluation = serde_json::from_str(&json).unwrap();
    assert_eq!(eval, rt);
}
...
fn demotion_evidence_item_serde_round_trip() {
    ...
    let rt: DemotionEvidenceItem = serde_json::from_str(&json).unwrap();
    assert_eq!(item, rt);
}
...
fn demotion_error_serde_round_trip() {
    ...
    for err in &errors {
        ...
        let rt: DemotionError = serde_json::from_str(&json).unwrap();
        assert_eq!(*err, rt);
    }
}
...
fn observation_result_serde_round_trip_no_trigger() {
    ...
    let rt: ObservationResult = serde_json::from_str(&json).unwrap();
    assert_eq!(or, rt);
}
...
fn performance_threshold_serde_round_trip() {
    ...
    let rt: PerformanceThreshold = serde_json::from_str(&json).unwrap();
    assert_eq!(pt, rt);
}
