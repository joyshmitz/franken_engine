...
| Parallel scheduler / merge | `src/parallel_parser.rs` tests | deterministic depth-aware split-point, scheduler-transcript replay, source-order merge, and merge-witness-hash assertions | `tests/parallel_parser_integration.rs`, `tests/parallel_interference_gate_integration.rs` | `scripts/run_parser_depth_partitioner_gate.sh`, `scripts/run_parser_scheduler_transcript_gate.sh`, `scripts/run_parser_merge_witness_gate.sh` + phase0/oracle backstops | parser-depth-partitioner + parser-scheduler-transcript + parser-merge-witness `run_manifest.json`, `events.jsonl`, `commands.txt` |
